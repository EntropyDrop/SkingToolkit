...
The versioned UV labels must be generated once before the first real-domain
semantic run:

```bash
./regenerate_v94_semantic_results.sh
./run_dense_semantic_training.sh
```

The regeneration command pins `runs/dense_uv_parser_v94/best.pt`, skips
existing valid work by default, writes each result atomically, and suppresses
all diagnostic images. Use `LIMIT=10` for a smoke test or
`SKIP_EXISTING=false` to deliberately rebuild every label.
...
By default the command recursively loads paired `*_edited.png` stage-one
renders and 64x64 `*_v94_result.png` UVs regenerated by the v94 baseline from
...
Pairing is strict: a historical `*_result.png` is never substituted when the
matching `*_v94_result.png` is absent. Override
`REAL_SEMANTIC_RESULT_SUFFIX` only when intentionally building a separately
versioned semantic dataset.
