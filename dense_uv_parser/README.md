...
front/back render pixels
  -> high-resolution CNN + frozen TIPSv2 spatial/multi-view context
...
The standard launcher uses `geometry_fit` plus a frozen `google/tipsv2-b14`
vision backbone. TIPSv2 is not trained. Its final 2D feature map is projected
through a 64-channel, zero-initialized residual adapter into the parser
bottleneck, while pooled front/back features retain the existing
zero-initialized multi-view FiLM path. The zero initialization makes the first
forward pass identical to the stable geometry parser and lets spatial semantic
corrections emerge only from supervised route errors.

Before TIPSv2 encoding, known background pixels are replaced by deterministic
neutral gray using the exact training/inference foreground mask. This prevents
randomized background colors from becoming semantic routing evidence. TIPSv2
is evaluated online in bounded chunks because caching raw
`channels x height x width` maps for 180,000 skins would require impractical
storage. With the default one-epoch run, online extraction performs the same
order of frozen-backbone work as building a cache and then training once.
The pooled semantic fusion remains 128 channels, 4 attention heads, and
1 transformer layer.
...
Install `transformers` and `safetensors` before training. The adapter prefers
native Transformers `AutoBackbone` support; releases that do not yet register
`model_type=tipsv2` automatically fall back to the official
`google/tipsv2-b14` repository's `trust_remote_code` `encode_image` interface.
That fallback reads its spatial `patch_tokens` directly and does not depend on
the repository's image-processor metadata. Reduce
`SEMANTIC_RUNTIME_BATCH_SIZE` if frozen TIPSv2 extraction runs out of memory.
For a geometry-only ablation, run:

```bash
SEMANTIC_BACKBONE=none CACHE_SIGLIP_GLOBALS=false ./run_dense_uv_parser_training.sh
```

The former cached global SigLIP2 configuration remains available for ablation
and old checkpoints:

```bash
SEMANTIC_BACKBONE=siglip2 ./run_dense_uv_parser_training.sh
```
...
local pixels, the configured view identity, TIPSv2 spatial features, and pooled
