...
Install a recent `transformers` version with `Tipsv2VisionBackbone` support and
`safetensors` before training. Reduce `SEMANTIC_RUNTIME_BATCH_SIZE` if frozen
TIPSv2 extraction runs out of memory. For a geometry-only ablation, run:
