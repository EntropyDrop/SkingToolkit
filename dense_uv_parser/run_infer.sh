...
case "$ROUTING_PROFILE" in
  balanced)
    ...
    DEFAULT_OUTER_UV_MIN_SOURCE_PIXELS="33"
  ...
  conservative)
    ...
    DEFAULT_ROUTE_CONFIDENCE_THRESHOLD="0.0"
    DEFAULT_ROUTE_MARGIN_THRESHOLD="0.0"
    DEFAULT_OUTER_ROUTE_CONFIDENCE_THRESHOLD="0.60"
    DEFAULT_OUTER_ROUTE_MARGIN_THRESHOLD="0.30"
    DEFAULT_OUTER_UV_MIN_COVERAGE="0.10"
    DEFAULT_OUTER_UV_MIN_SOURCE_PIXELS="33"
...
FOREGROUND_FLOOD_GRADIENT_TOLERANCE="${FOREGROUND_FLOOD_GRADIENT_TOLERANCE:-0.05}"
FOREGROUND_FLOOD_MAX_SEED_TOLERANCE="${FOREGROUND_FLOOD_MAX_SEED_TOLERANCE:-0.20}"
COLOR_BACKGROUND_TOLERANCE="${COLOR_BACKGROUND_TOLERANCE:-0.20}"
...
HEAD_OUTER_TOPOLOGY_RESCUE="${HEAD_OUTER_TOPOLOGY_RESCUE:-true}"
HEAD_OUTER_TOPOLOGY_AUTO_RELIABILITY="${HEAD_OUTER_TOPOLOGY_AUTO_RELIABILITY:-true}"
HEAD_OUTER_TOPOLOGY_MIN_PRECISION="${HEAD_OUTER_TOPOLOGY_MIN_PRECISION:-0.75}"
HEAD_OUTER_TOPOLOGY_MIN_RECALL="${HEAD_OUTER_TOPOLOGY_MIN_RECALL:-0.55}"
HEAD_OUTER_TOPOLOGY_SEMANTIC_THRESHOLD="${HEAD_OUTER_TOPOLOGY_SEMANTIC_THRESHOLD:-0.25}"
HEAD_OUTER_TOPOLOGY_RELAXED_ROUTE_THRESHOLD="${HEAD_OUTER_TOPOLOGY_RELAXED_ROUTE_THRESHOLD:-0.50}"
HEAD_OUTER_TOPOLOGY_RELAXED_SEMANTIC_THRESHOLD="${HEAD_OUTER_TOPOLOGY_RELAXED_SEMANTIC_THRESHOLD:-0.80}"
HEAD_OUTER_TOPOLOGY_SEMANTIC_ONLY_THRESHOLD="${HEAD_OUTER_TOPOLOGY_SEMANTIC_ONLY_THRESHOLD:-0.92}"
HEAD_OUTER_TOPOLOGY_RING_SEMANTIC_THRESHOLD="${HEAD_OUTER_TOPOLOGY_RING_SEMANTIC_THRESHOLD:-0.65}"
HEAD_OUTER_TOPOLOGY_MIN_SEED_NODES="${HEAD_OUTER_TOPOLOGY_MIN_SEED_NODES:-2}"
HEAD_OUTER_TOPOLOGY_COLOR_TOLERANCE="${HEAD_OUTER_TOPOLOGY_COLOR_TOLERANCE:-0.20}"
HEAD_OUTER_COMPLETION_THRESHOLD="${HEAD_OUTER_COMPLETION_THRESHOLD:-0.65}"
...
OUTER_UV_OCCUPANCY_AUTO_RELIABILITY="${OUTER_UV_OCCUPANCY_AUTO_RELIABILITY:-true}"
OUTER_UV_OCCUPANCY_MIN_PRECISION="${OUTER_UV_OCCUPANCY_MIN_PRECISION:-0.60}"
OUTER_UV_OCCUPANCY_MIN_RECALL="${OUTER_UV_OCCUPANCY_MIN_RECALL:-0.40}"
...
args=(
  ...
  --foreground_flood_gradient_tolerance "$FOREGROUND_FLOOD_GRADIENT_TOLERANCE"
  --foreground_flood_max_seed_tolerance "$FOREGROUND_FLOOD_MAX_SEED_TOLERANCE"
  ...
  --head_outer_topology_semantic_threshold "$HEAD_OUTER_TOPOLOGY_SEMANTIC_THRESHOLD"
  --head_outer_topology_relaxed_route_threshold "$HEAD_OUTER_TOPOLOGY_RELAXED_ROUTE_THRESHOLD"
  --head_outer_topology_relaxed_semantic_threshold "$HEAD_OUTER_TOPOLOGY_RELAXED_SEMANTIC_THRESHOLD"
  --head_outer_topology_semantic_only_threshold "$HEAD_OUTER_TOPOLOGY_SEMANTIC_ONLY_THRESHOLD"
  --head_outer_topology_ring_semantic_threshold "$HEAD_OUTER_TOPOLOGY_RING_SEMANTIC_THRESHOLD"
  --head_outer_topology_min_seed_nodes "$HEAD_OUTER_TOPOLOGY_MIN_SEED_NODES"
  --head_outer_topology_color_tolerance "$HEAD_OUTER_TOPOLOGY_COLOR_TOLERANCE"
  --head_outer_completion_threshold "$HEAD_OUTER_COMPLETION_THRESHOLD"
  ...
  --head_outer_topology_min_precision "$HEAD_OUTER_TOPOLOGY_MIN_PRECISION"
  --head_outer_topology_min_recall "$HEAD_OUTER_TOPOLOGY_MIN_RECALL"
...
if [[ "$OUTER_SILHOUETTE_CONSISTENCY" == "true" ]]; then
  ...
  args+=(--no_outer_silhouette_consistency)
fi

if [[ "$HEAD_OUTER_TOPOLOGY_RESCUE" == "true" ]]; then
  args+=(--head_outer_topology_rescue)
else
  args+=(--no_head_outer_topology_rescue)
fi

if [[ "$HEAD_OUTER_TOPOLOGY_AUTO_RELIABILITY" == "true" ]]; then
...
if [[ "$GEOMETRY_CROSS_VIEW_OUTER_CONSISTENCY" == "true" ]]; then
  ...
  args+=(--no_geometry_cross_view_outer_consistency)
fi
...
if [[ "$OUTER_UV_OCCUPANCY_AUTO_RELIABILITY" == "true" ]]; then
  args+=(--outer_uv_occupancy_auto_reliability)
else
  args+=(--no_outer_uv_occupancy_auto_reliability)
fi
args+=(
  --outer_uv_occupancy_min_precision "$OUTER_UV_OCCUPANCY_MIN_PRECISION"
  --outer_uv_occupancy_min_recall "$OUTER_UV_OCCUPANCY_MIN_RECALL"
)

if [[ "$OUTER_UV_COMPONENT_ROUTING" == "true" ]]; then
  args+=(--outer_uv_component_routing)
else
  args+=(--no_outer_uv_component_routing)
fi
...
if [[ -n "$OUTER_UV_OCCUPANCY_OUTPUT" ]]; then
  args+=(--outer_uv_occupancy_output "$OUTER_UV_OCCUPANCY_OUTPUT")
fi
...
if [[ "$FOREGROUND_METHOD" != "legacy" ]]; then
  ...
  echo "Foreground flood gradient tolerance: $FOREGROUND_FLOOD_GRADIENT_TOLERANCE"
  echo "Foreground flood max seed tolerance: $FOREGROUND_FLOOD_MAX_SEED_TOLERANCE"
...
if [[ -n "$OUTER_UV_OCCUPANCY_OUTPUT" ]]; then
  echo "Outer UV occupancy output: $OUTER_UV_OCCUPANCY_OUTPUT"
fi
if [[ -n "$HEAD_OUTER_STRUCTURE_OUTPUT" ]]; then
  echo "Head outer structure output: $HEAD_OUTER_STRUCTURE_OUTPUT"
fi
if [[ -n "$OUTPUT" ]]; then
