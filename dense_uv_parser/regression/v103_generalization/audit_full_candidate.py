...
root=Path('dense_uv_parser').resolve();run=root/'runs/v103_generalization_20260907';label=o.label or str(o.step);candidate=run/f'candidate_{label}';output=root/'output_history'/f'v103_generalization_{label}_20260907';rows=json.loads((candidate/'full_inputs.json').read_text())
...
if o.repeat:
 repeat=root/'output_history'/f'v103_generalization_{label}_20260907_repeat';repeats={json.loads(p.read_text())['input_sha256']:p for p in repeat.glob('*/manifest.json')}
report={'checkpoint':str(candidate/'parser.pt'),'checkpoint_sha256':sha(candidate/'parser.pt'),'cases':{},'scope':'3 real training identities are fit checks; 28 real development images have no gradients. Includes the 2 feedback images and 12 additional fixed selections. Whole-image review is required alongside scoped checks.','release_promoted':False}
arrays={}
for row in rows:
...
for page in range(0,len(ordered),3):
 ...
 for ri,row in enumerate(ordered[page:page+3]):
  for ci,(label,path) in enumerate([('Input',row['input']),('Previous v103',Path(row['old_output'])/'simple_inpaint_render.png'),('Candidate',Path(report['cases'][row['name']]['output'])/'simple_inpaint_render.png')]):
