...
for path in paths:
 c=load(path);m=FinalHeadUVDecoder(**c['final_head_uv_config']).cuda().eval();m.load_state_dict(c['final_head_uv_state']);step=c.get('final_head_uv_step');label='v103' if path==paths[0] else path.stem;stats={}
 for offset in range(0,len(validation),8):
  ...
  for i,r in enumerate(rows):
   ...
   s=stats.setdefault(cohort,{'cases':0,'tp':0,'fp':0,'fn':0});t=r['target'][0,3]>.5;ids=m.ids[m.outer].cpu();a=pred[i].flatten()[ids];b=t.flatten()[ids]
   s['cases']+=1;s['tp']+=int((a&b).sum());s['fp']+=int((a&~b).sum());s['fn']+=int((~a&b).sum())
 ...
 score=sum((s['fp']+s['fn'])*(1 if k.startswith('deployed_') else .25) for k,s in stats.items())
...
report['selection_rule']='Lowest deployed validation FP+FN plus 0.25 times replay FP+FN, after serialization/roundtrip and existing scoped development regression guards. v101 hair guard is only protection from a large known regression.'
