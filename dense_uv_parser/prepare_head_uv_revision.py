...
def author(original,seed,family):
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output-dir',type=Path,required=True);p.add_argument('--train-count',type=int,default=128);p.add_argument('--validation-count',type=int,default=32);o=p.parse_args()
    ...
    manifest['revision']='stratified_head_20260907';manifest['parent_cache']=str(old)
    ...
    start=time.time();counts={};samples={}
    for group,amount,start_index in [('train',o.train_count,128),('validation',o.validation_count,32)]:
        folder=out/group;folder.mkdir(exist_ok=True);samples[group]=Counter();counts[group]=Counter()
        for i in range(amount):
            ...
            if path.exists():
                ...
                item=author(load_skin(source),seed,family);uv=item['uv'][None].cuda();images=[];foreground=[]
                ...
                save_case(temporary,result,uv,item['labels'][None],item.get('symmetric',False),{'source':source,'family':family,'seed':seed,'kind':'rendered_ground_truth','split':group,'actual_parser_cache':True})
