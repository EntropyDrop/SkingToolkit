...
SEMANTIC_GATE_RADIUS="${SEMANTIC_GATE_RADIUS:-1}"
SPLAT_COLOR_AGGREGATION="${SPLAT_COLOR_AGGREGATION:-quantized_mode}"
SPLAT_COLOR_MODE_BITS="${SPLAT_COLOR_MODE_BITS:-5}"
SPLAT_COLOR_MODE_CONFIDENCE_RATIO="${SPLAT_COLOR_MODE_CONFIDENCE_RATIO:-0.85}"
...
python train.py \
  ...
  --splat_color_mode_bits "$SPLAT_COLOR_MODE_BITS" \
  --splat_color_mode_confidence_ratio "$SPLAT_COLOR_MODE_CONFIDENCE_RATIO" \
  "${augment_args[@]}" \
  "${background_args[@]}" \
  "${semantic_gate_args[@]}" \
  "${uv_class_args[@]}" \
