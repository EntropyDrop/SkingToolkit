...
SIGLIP_CACHE_DIR="${SIGLIP_CACHE_DIR:-cache/semantic_dense_parser_siglip2_spatial_${MAPPINGS_SIZE}_${MAX_SAMPLES}_${SEMANTIC_VIEW_TAG}}"
if [[ -n "$PRIVILEGED_VIEWS" ]]; then
  DEFAULT_SIGLIP_CACHE_BATCH_SIZE=16
else
  DEFAULT_SIGLIP_CACHE_BATCH_SIZE=32
fi
SIGLIP_CACHE_BATCH_SIZE="${SIGLIP_CACHE_BATCH_SIZE:-$DEFAULT_SIGLIP_CACHE_BATCH_SIZE}"
...
DENSE_SEMANTIC_TARGET_VERSION="${DENSE_SEMANTIC_TARGET_VERSION:-3}"
HIERARCHICAL_DENSE_SEMANTICS="${HIERARCHICAL_DENSE_SEMANTICS:-false}"
PREDICT_HEAD_OUTER_STRUCTURE="${PREDICT_HEAD_OUTER_STRUCTURE:-true}"
HEAD_OUTER_STRUCTURE_MODE="${HEAD_OUTER_STRUCTURE_MODE:-projected}"
HEAD_OUTER_PROJECTED_INPUT_VERSION="${HEAD_OUTER_PROJECTED_INPUT_VERSION:-5}"
PREDICT_OUTER_UV_OCCUPANCY="${PREDICT_OUTER_UV_OCCUPANCY:-true}"
OUTER_UV_FEATURE_CHANNELS="${OUTER_UV_FEATURE_CHANNELS:-64}"
OUTER_UV_TOPOLOGY_CHANNELS="${OUTER_UV_TOPOLOGY_CHANNELS:-96}"
OUTER_UV_TOPOLOGY_LAYERS="${OUTER_UV_TOPOLOGY_LAYERS:-4}"
...
# Four training renders per skin have approximately twice the activation
# footprint of the two-view inference configuration.
...
REAL_SEMANTIC_PIXEL_RGB_TOLERANCE="${REAL_SEMANTIC_PIXEL_RGB_TOLERANCE:-0.12}"
REAL_SEMANTIC_MISMATCH_DILATION="${REAL_SEMANTIC_MISMATCH_DILATION:-1}"
...
OUTER_ROUTE_CONFIDENCE_THRESHOLD="${OUTER_ROUTE_CONFIDENCE_THRESHOLD:-0.60}"
OUTER_ROUTE_MARGIN_THRESHOLD="${OUTER_ROUTE_MARGIN_THRESHOLD:-0.30}"
OUTER_UV_MIN_COVERAGE="${OUTER_UV_MIN_COVERAGE:-0.10}"
OUTER_UV_MIN_SOURCE_PIXELS="${OUTER_UV_MIN_SOURCE_PIXELS:-2}"
...
OUTER_SILHOUETTE_DILATION="${OUTER_SILHOUETTE_DILATION:-0}"
OUTER_SILHOUETTE_MIN_PIXELS="${OUTER_SILHOUETTE_MIN_PIXELS:-1}"
OUTER_GEOMETRY_RESCUE="${OUTER_GEOMETRY_RESCUE:-true}"
OUTER_RESCUE_CONFIDENCE_THRESHOLD="${OUTER_RESCUE_CONFIDENCE_THRESHOLD:-0.40}"
OUTER_RESCUE_MARGIN_THRESHOLD="${OUTER_RESCUE_MARGIN_THRESHOLD:-0.15}"
OUTER_RESCUE_MIN_COVERAGE="${OUTER_RESCUE_MIN_COVERAGE:-0.05}"
...
GEOMETRY_CROSS_VIEW_OUTER_CONSISTENCY="${GEOMETRY_CROSS_VIEW_OUTER_CONSISTENCY:-true}"
...
LAMBDA_CROSS_VIEW_OUTER_VISIBILITY="${LAMBDA_CROSS_VIEW_OUTER_VISIBILITY:-0.25}"
CROSS_VIEW_OUTER_CONSISTENCY_LOSS_WEIGHT="${CROSS_VIEW_OUTER_CONSISTENCY_LOSS_WEIGHT:-0.25}"
if [[ -n "$PRIVILEGED_VIEWS" ]]; then
  DEFAULT_PRIVILEGED_VIEW_DISTILLATION=0.25
else
  DEFAULT_PRIVILEGED_VIEW_DISTILLATION=0.0
fi
LAMBDA_PRIVILEGED_VIEW_DISTILLATION="${LAMBDA_PRIVILEGED_VIEW_DISTILLATION:-$DEFAULT_PRIVILEGED_VIEW_DISTILLATION}"
PRIVILEGED_VIEW_TEACHER_CONFIDENCE="${PRIVILEGED_VIEW_TEACHER_CONFIDENCE:-0.70}"
...
if [[ "$SIGLIP_TEXT_PROMPT_FUSION" == "true" ]]; then
  if [[ "$DENSE_SEMANTIC_TARGET_VERSION" != "1" ]]; then
    DEFAULT_TEXT_PROMPT_ROUTE_WEIGHT=0.25
  else
    DEFAULT_TEXT_PROMPT_ROUTE_WEIGHT=0.35
  fi
else
...
if [[ "$DENSE_SEMANTIC_TARGET_VERSION" != "1" ]]; then
  DEFAULT_DENSE_SEMANTICS_WEIGHT=0.40
else
...
DENSE_SEMANTIC_OUTER_UNION_WEIGHT="${DENSE_SEMANTIC_OUTER_UNION_WEIGHT:-0.0}"
if [[ "$PREDICT_HEAD_OUTER_STRUCTURE" == "true" ]]; then
  DEFAULT_HEAD_OUTER_PRESENCE_WEIGHT=0.10
  DEFAULT_HEAD_OUTER_COVERAGE_WEIGHT=0.10
  DEFAULT_HEAD_OUTER_OCCUPANCY_WEIGHT=0.55
  DEFAULT_HEAD_OUTER_TOPOLOGY_WEIGHT=0.25
  DEFAULT_HEAD_OUTER_SYMMETRY_WEIGHT=0.05
  DEFAULT_HEAD_OUTER_COMPONENT_HARD_RECALL_WEIGHT=0.20
  DEFAULT_HEAD_OUTER_SPARSE_RECALL_WEIGHT=0.20
  DEFAULT_HEAD_OUTER_SYMMETRY_SCORE_WEIGHT=0.10
  DEFAULT_HEAD_OUTER_RING_WORST_FACE_RECALL_WEIGHT=0.25
  DEFAULT_HEAD_OUTER_OPEN_TOP_SHAPE_WEIGHT=0.40
  DEFAULT_HEAD_OUTER_ACCESSORY_CLASSIFICATION_WEIGHT=0.15
  DEFAULT_HEAD_OUTER_ROUTE_CONNECTIVITY_WEIGHT=0.30
  DEFAULT_HEAD_OUTER_ROUTE_SYMMETRY_WEIGHT=0.25
else
  DEFAULT_HEAD_OUTER_PRESENCE_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_COVERAGE_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_OCCUPANCY_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_TOPOLOGY_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_SYMMETRY_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_COMPONENT_HARD_RECALL_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_SPARSE_RECALL_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_SYMMETRY_SCORE_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_RING_WORST_FACE_RECALL_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_OPEN_TOP_SHAPE_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_ACCESSORY_CLASSIFICATION_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_ROUTE_CONNECTIVITY_WEIGHT=0.0
  DEFAULT_HEAD_OUTER_ROUTE_SYMMETRY_WEIGHT=0.0
fi
LAMBDA_HEAD_OUTER_PRESENCE="${LAMBDA_HEAD_OUTER_PRESENCE:-$DEFAULT_HEAD_OUTER_PRESENCE_WEIGHT}"
LAMBDA_HEAD_OUTER_COVERAGE="${LAMBDA_HEAD_OUTER_COVERAGE:-$DEFAULT_HEAD_OUTER_COVERAGE_WEIGHT}"
LAMBDA_HEAD_OUTER_OCCUPANCY="${LAMBDA_HEAD_OUTER_OCCUPANCY:-$DEFAULT_HEAD_OUTER_OCCUPANCY_WEIGHT}"
HEAD_OUTER_OCCUPANCY_DICE_WEIGHT="${HEAD_OUTER_OCCUPANCY_DICE_WEIGHT:-0.25}"
HEAD_OUTER_OCCUPANCY_POSITIVE_BALANCE="${HEAD_OUTER_OCCUPANCY_POSITIVE_BALANCE:-0.55}"
HEAD_OUTER_HARD_NEGATIVE_FRACTION="${HEAD_OUTER_HARD_NEGATIVE_FRACTION:-0.10}"
HEAD_OUTER_HARD_NEGATIVE_WEIGHT="${HEAD_OUTER_HARD_NEGATIVE_WEIGHT:-0.50}"
HEAD_OUTER_PRECISION_WEIGHT="${HEAD_OUTER_PRECISION_WEIGHT:-0.50}"
LAMBDA_HEAD_OUTER_TOPOLOGY="${LAMBDA_HEAD_OUTER_TOPOLOGY:-$DEFAULT_HEAD_OUTER_TOPOLOGY_WEIGHT}"
LAMBDA_HEAD_OUTER_SYMMETRY="${LAMBDA_HEAD_OUTER_SYMMETRY:-$DEFAULT_HEAD_OUTER_SYMMETRY_WEIGHT}"
LAMBDA_HEAD_OUTER_COMPONENT_HARD_RECALL="${LAMBDA_HEAD_OUTER_COMPONENT_HARD_RECALL:-$DEFAULT_HEAD_OUTER_COMPONENT_HARD_RECALL_WEIGHT}"
LAMBDA_HEAD_OUTER_SPARSE_RECALL="${LAMBDA_HEAD_OUTER_SPARSE_RECALL:-$DEFAULT_HEAD_OUTER_SPARSE_RECALL_WEIGHT}"
LAMBDA_HEAD_OUTER_SYMMETRY_SCORE="${LAMBDA_HEAD_OUTER_SYMMETRY_SCORE:-$DEFAULT_HEAD_OUTER_SYMMETRY_SCORE_WEIGHT}"
LAMBDA_HEAD_OUTER_RING_WORST_FACE_RECALL="${LAMBDA_HEAD_OUTER_RING_WORST_FACE_RECALL:-$DEFAULT_HEAD_OUTER_RING_WORST_FACE_RECALL_WEIGHT}"
LAMBDA_HEAD_OUTER_OPEN_TOP_SHAPE="${LAMBDA_HEAD_OUTER_OPEN_TOP_SHAPE:-$DEFAULT_HEAD_OUTER_OPEN_TOP_SHAPE_WEIGHT}"
...
if [[ "$HEAD_OUTER_PROJECTED_INPUT_VERSION" -ge 5 ]]; then
  DEFAULT_HEAD_EYE_OCCUPANCY_WEIGHT=0.45
  DEFAULT_HEAD_EYE_PRESENCE_WEIGHT=0.15
  DEFAULT_HEAD_EYE_SYMMETRY_WEIGHT=0.10
  DEFAULT_HEAD_EYE_ROUTE_CONNECTIVITY_WEIGHT=0.25
  DEFAULT_HEAD_EYE_ROUTE_SYMMETRY_WEIGHT=0.15
else
  DEFAULT_HEAD_EYE_OCCUPANCY_WEIGHT=0.0
  DEFAULT_HEAD_EYE_PRESENCE_WEIGHT=0.0
  DEFAULT_HEAD_EYE_SYMMETRY_WEIGHT=0.0
  DEFAULT_HEAD_EYE_ROUTE_CONNECTIVITY_WEIGHT=0.0
  DEFAULT_HEAD_EYE_ROUTE_SYMMETRY_WEIGHT=0.0
fi
LAMBDA_HEAD_EYE_OCCUPANCY="${LAMBDA_HEAD_EYE_OCCUPANCY:-$DEFAULT_HEAD_EYE_OCCUPANCY_WEIGHT}"
HEAD_EYE_OCCUPANCY_DICE_WEIGHT="${HEAD_EYE_OCCUPANCY_DICE_WEIGHT:-0.50}"
HEAD_EYE_OCCUPANCY_POSITIVE_BALANCE="${HEAD_EYE_OCCUPANCY_POSITIVE_BALANCE:-0.45}"
HEAD_EYE_HARD_FRACTION="${HEAD_EYE_HARD_FRACTION:-0.25}"
HEAD_EYE_HARD_POSITIVE_WEIGHT="${HEAD_EYE_HARD_POSITIVE_WEIGHT:-0.50}"
HEAD_EYE_HARD_NEGATIVE_WEIGHT="${HEAD_EYE_HARD_NEGATIVE_WEIGHT:-1.00}"
LAMBDA_HEAD_EYE_PRESENCE="${LAMBDA_HEAD_EYE_PRESENCE:-$DEFAULT_HEAD_EYE_PRESENCE_WEIGHT}"
LAMBDA_HEAD_EYE_SYMMETRY="${LAMBDA_HEAD_EYE_SYMMETRY:-$DEFAULT_HEAD_EYE_SYMMETRY_WEIGHT}"
LAMBDA_HEAD_EYE_ROUTE_CONNECTIVITY="${LAMBDA_HEAD_EYE_ROUTE_CONNECTIVITY:-$DEFAULT_HEAD_EYE_ROUTE_CONNECTIVITY_WEIGHT}"
LAMBDA_HEAD_EYE_ROUTE_SYMMETRY="${LAMBDA_HEAD_EYE_ROUTE_SYMMETRY:-$DEFAULT_HEAD_EYE_ROUTE_SYMMETRY_WEIGHT}"
LAMBDA_HEAD_OUTER_ROUTE_CONNECTIVITY="${LAMBDA_HEAD_OUTER_ROUTE_CONNECTIVITY:-$DEFAULT_HEAD_OUTER_ROUTE_CONNECTIVITY_WEIGHT}"
LAMBDA_HEAD_OUTER_ROUTE_SYMMETRY="${LAMBDA_HEAD_OUTER_ROUTE_SYMMETRY:-$DEFAULT_HEAD_OUTER_ROUTE_SYMMETRY_WEIGHT}"
...
LAMBDA_OUTER_UV_OCCUPANCY="${LAMBDA_OUTER_UV_OCCUPANCY:-0.55}"
...
OUTER_HARD_NEGATIVE_FRACTION="${OUTER_HARD_NEGATIVE_FRACTION:-0.05}"
OUTER_HARD_NEGATIVE_WEIGHT="${OUTER_HARD_NEGATIVE_WEIGHT:-0.50}"
LAMBDA_OUTER_COMPONENT_RECALL="${LAMBDA_OUTER_COMPONENT_RECALL:-0.20}"
LAMBDA_OUTER_COMPONENT_FALSE_POSITIVE="${LAMBDA_OUTER_COMPONENT_FALSE_POSITIVE:-0.10}"
LAMBDA_OUTER_TOPOLOGY="${LAMBDA_OUTER_TOPOLOGY:-0.25}"
LAMBDA_OUTER_NEGATIVE_TOPOLOGY="${LAMBDA_OUTER_NEGATIVE_TOPOLOGY:-0.15}"
LAMBDA_ROUTE_OCCUPANCY_AGREEMENT="${LAMBDA_ROUTE_OCCUPANCY_AGREEMENT:-0.30}"
...
LAMBDA_SOFT_UV_RGB="${LAMBDA_SOFT_UV_RGB:-0.50}"
LAMBDA_SOFT_UV_ALPHA="${LAMBDA_SOFT_UV_ALPHA:-0.60}"
LAMBDA_SOFT_UV_INNER_RECALL="${LAMBDA_SOFT_UV_INNER_RECALL:-0.75}"
LAMBDA_SOFT_UV_OUTER_RECALL="${LAMBDA_SOFT_UV_OUTER_RECALL:-0.75}"
SOFT_UV_RECALL_HARD_FRACTION="${SOFT_UV_RECALL_HARD_FRACTION:-0.10}"
SOFT_UV_RECALL_HARD_WEIGHT="${SOFT_UV_RECALL_HARD_WEIGHT:-0.50}"
LAMBDA_RENDER_RGB="${LAMBDA_RENDER_RGB:-0.40}"
LAMBDA_RENDER_ALPHA="${LAMBDA_RENDER_ALPHA:-0.50}"
...
head_outer_structure_args=()
if [[ "$PREDICT_HEAD_OUTER_STRUCTURE" == "true" ]]; then
  head_outer_structure_args=(--predict_head_outer_structure)
else
  head_outer_structure_args=(--no_predict_head_outer_structure)
fi
...
if [[ -n "$REAL_SEMANTIC_DATA_DIR" ]]; then
  real_semantic_args=(
    ...
    --real_semantic_pixel_rgb_tolerance "$REAL_SEMANTIC_PIXEL_RGB_TOLERANCE"
    --real_semantic_mismatch_dilation "$REAL_SEMANTIC_MISMATCH_DILATION"
  )
  paired_cache_args=(
...
if [[ "$HIERARCHICAL_DENSE_SEMANTICS" == "true" ]]; then
  semantic_args+=(--hierarchical_dense_semantics)
else
  semantic_args+=(--no_hierarchical_dense_semantics)
fi
if [[ "$SEMANTIC_BACKBONE" != "none" ]]; then
  semantic_args+=(
    ...
    --dense_semantic_target_version "$DENSE_SEMANTIC_TARGET_VERSION"
...
exec python train.py \
  ...
  --lambda_privileged_view_distillation "$LAMBDA_PRIVILEGED_VIEW_DISTILLATION" \
  --privileged_view_teacher_confidence "$PRIVILEGED_VIEW_TEACHER_CONFIDENCE" \
  ...
  --dense_semantic_outer_union_weight "$DENSE_SEMANTIC_OUTER_UNION_WEIGHT" \
  ...
  --head_outer_occupancy_dice_weight "$HEAD_OUTER_OCCUPANCY_DICE_WEIGHT" \
  --head_outer_occupancy_positive_balance "$HEAD_OUTER_OCCUPANCY_POSITIVE_BALANCE" \
  --head_outer_hard_negative_fraction "$HEAD_OUTER_HARD_NEGATIVE_FRACTION" \
  --head_outer_hard_negative_weight "$HEAD_OUTER_HARD_NEGATIVE_WEIGHT" \
  --head_outer_precision_weight "$HEAD_OUTER_PRECISION_WEIGHT" \
  --lambda_head_outer_topology "$LAMBDA_HEAD_OUTER_TOPOLOGY" \
  --lambda_head_outer_symmetry "$LAMBDA_HEAD_OUTER_SYMMETRY" \
  --lambda_head_outer_component_hard_recall "$LAMBDA_HEAD_OUTER_COMPONENT_HARD_RECALL" \
  --lambda_head_outer_sparse_recall "$LAMBDA_HEAD_OUTER_SPARSE_RECALL" \
  --lambda_head_outer_symmetry_score "$LAMBDA_HEAD_OUTER_SYMMETRY_SCORE" \
  --lambda_head_outer_ring_worst_face_recall "$LAMBDA_HEAD_OUTER_RING_WORST_FACE_RECALL" \
  --lambda_head_outer_open_top_shape "$LAMBDA_HEAD_OUTER_OPEN_TOP_SHAPE" \
  --lambda_head_outer_accessory_classification "$LAMBDA_HEAD_OUTER_ACCESSORY_CLASSIFICATION" \
  --lambda_head_eye_occupancy "$LAMBDA_HEAD_EYE_OCCUPANCY" \
  --head_eye_occupancy_dice_weight "$HEAD_EYE_OCCUPANCY_DICE_WEIGHT" \
  --head_eye_occupancy_positive_balance "$HEAD_EYE_OCCUPANCY_POSITIVE_BALANCE" \
  --head_eye_hard_fraction "$HEAD_EYE_HARD_FRACTION" \
  --head_eye_hard_positive_weight "$HEAD_EYE_HARD_POSITIVE_WEIGHT" \
  --head_eye_hard_negative_weight "$HEAD_EYE_HARD_NEGATIVE_WEIGHT" \
  --lambda_head_eye_presence "$LAMBDA_HEAD_EYE_PRESENCE" \
  --lambda_head_eye_symmetry "$LAMBDA_HEAD_EYE_SYMMETRY" \
  --lambda_head_eye_route_connectivity "$LAMBDA_HEAD_EYE_ROUTE_CONNECTIVITY" \
  --lambda_head_eye_route_symmetry "$LAMBDA_HEAD_EYE_ROUTE_SYMMETRY" \
  --lambda_head_outer_route_connectivity "$LAMBDA_HEAD_OUTER_ROUTE_CONNECTIVITY" \
  --lambda_head_outer_route_symmetry "$LAMBDA_HEAD_OUTER_ROUTE_SYMMETRY" \
  --head_outer_route_hard_fraction "$HEAD_OUTER_ROUTE_HARD_FRACTION" \
